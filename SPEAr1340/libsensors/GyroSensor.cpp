...
int GyroSensor::setRange(int32_t range)
{
    int fd;
    strcpy(&input_sysfs_path[input_sysfs_path_len], "range");
    LOGD("XXXXXXXXX gyroscope setting range %s %d", input_sysfs_path, range);
    fd = open(input_sysfs_path, O_RDWR);
    if (fd >= 0) {
        char buf[80];
        sprintf(buf, "%d", range);
