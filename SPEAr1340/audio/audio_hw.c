...
#define LOG_TAG "audio_hw_primary_SPEAr1340"
//#define LOG_NDEBUG 0
...
struct mixer_ctls
{
	...
	struct mixer_ctl *left_vol;
	struct mixer_ctl *right_vol;
	struct mixer_ctl *master_mute;
};

struct spear_audio_device {
    ...
    int in_call;
    float voice_volume;
    struct spear_stream_in *active_input;
    struct spear_stream_out *active_output;
    bool mic_mute;
    int tty_mode;
...
static void select_output_device(struct spear_audio_device *adev);
static void select_input_device(struct spear_audio_device *adev);
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume);
static int do_input_standby(struct spear_stream_in *in);
static int do_output_standby(struct spear_stream_out *out);
...
static int start_call(struct spear_audio_device *adev)
{
	return -ENOTSUP;
}

static void end_call(struct spear_audio_device *adev)
{
...
static void set_incall_device(struct spear_audio_device *adev)
{
    int device_type;
...
static void set_input_volumes(struct spear_audio_device *adev, int main_mic_on,
...
static void set_output_volumes(struct spear_audio_device *adev, bool tty_volume)
{
	return;
#if 0
    unsigned int channel;
...
static void force_all_standby(struct spear_audio_device *adev)
{
    struct spear_stream_in *in;
    struct spear_stream_out *out;
...
static void select_mode(struct spear_audio_device *adev)
{
	return;
}
...
{
	int speaker_on = adev->devices & AUDIO_DEVICE_OUT_SPEAKER;
	mixer_ctl_set_enum_by_string(adev->mixer_ctls.master_mute,
			speaker_on ? "Off" : "On");
}
...
static int start_output_stream(struct spear_stream_out *out)
{
    struct spear_audio_device *adev = out->dev;
    unsigned int card = CARD_SPEAR_DEFAULT;
    unsigned int port = I2S_PLAYBACK;
    ...
    if (adev->devices & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET)
        port = SPDIF_PLAYBACK; // TODO: fix case
    else if(adev->devices & AUDIO_DEVICE_OUT_AUX_DIGITAL) {
        // TODO:
    }

    out->write_threshold = 0;
    /* start when buffer is full */
    out->config.start_threshold = pcm_config_out.period_size * (pcm_config_out.period_count) / 2;
    /* play out buffer */
    out->config.stop_threshold = pcm_config_out.period_size * (pcm_config_out.period_count);

    out->config.silence_threshold = 0;
    /* allow transfer to start when there is at least one period of samples */
    out->config.avail_min = pcm_config_out.period_size;
    /* default to low power - will be corrected before first write in out_write
     * in tinyalsa
     */
    out->low_power = 1;
...
static int check_input_parameters(uint32_t sample_rate, int format, int channel_count)
{
    ...
    LOGE("%s fs=%d, fmt=%d, chn=%d", __func__, sample_rate, format, channel_count);

    /* Any SR will do, we'll record @ 48 kHz and do an SRC in userspace */
...
static void add_echo_reference(struct spear_stream_out *out,
...
static void remove_echo_reference(struct spear_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct spear_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct spear_audio_device *adev,
...
static int get_playback_delay(struct spear_stream_out *out,
                       size_t frames,
...
{
    struct spear_stream_out *out = (struct spear_stream_out *)stream;
...
static int start_input_stream(struct spear_stream_in *in)
{
    int ret = 0;
    struct spear_audio_device *adev = in->dev;
    ...
    in->pcm = pcm_open(0, I2S_RECORD, PCM_IN, &in->config);
...
{
    struct spear_stream_in *in = (struct spear_stream_in *)stream;
...
static void get_capture_delay(struct spear_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct spear_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct spear_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct spear_stream_in *)((char *)buffer_provider -
                                   offsetof(struct spear_stream_in, buf_provider));
...
static ssize_t read_frames(struct spear_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct spear_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
{
    struct spear_audio_device *ladev = (struct spear_audio_device *)dev;
    struct spear_stream_out *out;
    int ret;

    out = (struct spear_stream_out *)calloc(1, sizeof(struct spear_stream_out));
    if (!out)
        return -ENOMEM;
...
{
    struct spear_audio_device *adev = (struct spear_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    in = (struct spear_stream_in *)calloc(1, sizeof(struct spear_stream_in));
    if (!in)
        return -ENOMEM;
    ...
    /* STA529 only supports stereo recording. Always record stereo. */
...
{
    struct spear_audio_device *adev = (struct spear_audio_device *)device;
...
static int adev_open(const hw_module_t* module, const char* name,
                     hw_device_t** device)
{
    struct spear_audio_device *adev;
    int ret;

    if (strcmp(name, AUDIO_HARDWARE_INTERFACE) != 0)
        return -EINVAL;

    adev = calloc(1, sizeof(struct spear_audio_device));
    if (!adev)
        return -ENOMEM;
	...
	adev->mixer_ctls.left_vol = mixer_get_ctl_by_name(adev->mixer,
			MIXER_LEFT_PLAYBACK_VOLUME);
	adev->mixer_ctls.right_vol = mixer_get_ctl_by_name(adev->mixer,
			MIXER_RIGHT_PLAYBACK_VOLUME);
	adev->mixer_ctls.master_mute = mixer_get_ctl_by_name(adev->mixer,
			MIXER_MASTER_MUTE);

    if (!adev->mixer_ctls.master_vol || !adev->mixer_ctls.left_vol ||
        !adev->mixer_ctls.right_vol || !adev->mixer_ctls.master_mute) {
        mixer_close(adev->mixer);
        free(adev);
